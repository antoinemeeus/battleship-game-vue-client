        ...
        >
          ...
          >
            ...
            >
              ...
              <!-- <SalvoMissiles
                ref="salvoM"
                :salvoSize="homeNextSalvoPositions.length"
                :missileNumber="maxSalvoSize"
              ></SalvoMissiles> -->
            </v-layout>
          </v-flex>
          <v-flex shrink>
          ...
          <v-layout row>
            <v-flex shrink>
              ...
              <Grid
                ...
                @mousemove.native="missileFollowMouse"
                @mouseleave.native="missileLeave"
              >
              </Grid>
            </v-flex>
            <v-flex
...
export default {
  ...
  methods: {
    ...
    missileFollowMouse(event) {
      if (this.gameFinished) return;
      if (!this.$refs.salvoM) return;
      let element = this.$refs.salvoM.$el;
      element.style.position = "absolute";
      element.style["z-index"] = 10;
      element.style.top = event.pageY - 50 + "px";
      element.style.left = event.pageX + 15 + "px";
      element.childNodes.forEach(child =>
        child.firstChild.classList.add("image-missile-small")
      );
    },
    missileLeave(event) {
      if (!this.$refs.salvoM) return;
      ...
      element.style.left = "";
      element.childNodes.forEach(child =>
        child.firstChild.classList.remove("image-missile-small")
      );
    },
